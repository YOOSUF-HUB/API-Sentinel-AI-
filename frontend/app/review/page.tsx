  ...
  if (!docId) {
    return (
      <div className="space-y-4 rounded-xl border border-ink-600 bg-ink-800 p-6 text-center">
        ...
        <Link
          href="/"
          className="inline-block rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-ink-900 transition hover:bg-accent-muted"
        >
          ← Back to upload
        </Link>
      </div>
...
export default function ReviewPage() {
  return (
    <Suspense
      fallback={
        <div className="text-sm text-slate-500">Loading review…</div>
      }
    >
      <ReviewContent />
    </Suspense>
